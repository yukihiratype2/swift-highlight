...
grammar_path="$project_root/Sources/HighlightSwift/Resources/Grammars"
...
package_name="HighlightSwift-$version"
package_path="$work_path/$package_name"

mkdir -p "$package_path" "$output_path"
git -C "$project_root" archive HEAD | tar -x -C "$package_path"
mkdir -p "$package_path/Sources/HighlightSwift/Resources"
cp -R "$grammar_path" "$package_path/Sources/HighlightSwift/Resources/Grammars"
...
tar -czf "$output_path/HighlightSwift-Grammars-$version.tar.gz" \
  -C "$project_root/Sources/HighlightSwift/Resources" Grammars
...
(
  ...
  sha256sum \
    "$package_name.tar.gz" \
    "$package_name.zip" \
    "HighlightSwift-Grammars-$version.tar.gz" > SHA256SUMS
)
